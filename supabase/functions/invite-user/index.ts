...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // console.log('invite-user: Received invitation request for email:', invited_email); // REMOVED
    // console.log('invite-user: DEBUG - Received invited_email from request body:', invited_email); // REMOVED
    ...
    // console.log('invite-user: Inviting user ID:', invitingUserId); // REMOVED
    // console.log('invite-user: Inviting user email:', invitingUserEmail); // REMOVED
    ...
    // console.log('invite-user: Inviter subscription ID:', subscriptionId); // REMOVED
    ...
    // console.log('invite-user: Max users for subscription:', maxUsers); // REMOVED
    ...
    // console.log('invite-user: DEBUG - Result of supabase.auth.admin.listUsers for invited_email:', existingAuthUsers); // REMOVED
    ...
    // console.log('invite-user: DEBUG - Final invitedUserId determined:', invitedUserId); // REMOVED
    ...
    if (invitedUserId) {
      // console.log('invite-user: Invited email belongs to an existing user. Checking their membership status...'); // REMOVED
      ...
      if (existingMembership && existingMembership.status === 'active') {
        ...
        // console.log('invite-user: Invited user is already an active member of this subscription. No action needed.'); // REMOVED
      ...
      // console.log('invite-user: User exists but is not an active member of this subscription. Checking limits and upserting...'); // REMOVED
      ...
      // console.log('invite-user: Current members count:', currentMembersCount); // REMOVED
      ...
      // console.log('invite-user: Upserted membership for existing user:', upsertedMembership.id); // REMOVED
      ...
      // console.log('invite-user: Invited user does NOT exist in auth.users. Proceeding to check limits and insert new membership with user_id: null...'); // REMOVED
      const { count: currentMembersCount, error: countError } = await supabase
        .from('subscription_memberships')
        .select('id', { count: 'exact' })
        .eq('subscription_id', subscriptionId)
      ...
      // console.log('invite-user: Current members count (unregistered user path):', currentMembersCount); // REMOVED
      ...
      // console.log('invite-user: Successfully inserted new membership for unregistered user:', newMembership.id); // REMOVED
    ...
    // console.log('invite-user: Generated invitation URL:', acceptInvitationUrl); // REMOVED
    ...
    if (emailFnInvokeError) {
      ...
      // console.log('invite-user: send-invitation-email Edge Function invoked successfully.'); // REMOVED
    }
