...
Deno.serve(async (req) => {
  ...
  try {
    const { recipientEmail, recipientName, subject, htmlBody, userPreferredLanguage } = await req.json();

    if (!recipientEmail || !subject || !htmlBody || !userPreferredLanguage) {
      return corsResponse({ error: getTranslatedMessage('message_missing_required_fields', userPreferredLanguage) }, 400);
    }

    // No direct user authentication needed here, as it's invoked internally by other Edge Functions.
    ...
    try {
      ...
      if (error) {
        console.error('Error sending generic notification email via Resend:', error);
      ...
      console.error('Caught unexpected error during generic notification email sending:', emailSendError);
    ...
    console.error('Error in send-generic-notification-email Edge Function:', error);
