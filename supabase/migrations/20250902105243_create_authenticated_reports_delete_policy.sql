...
BEGIN
    ...
    -- Add a comment to the policy for clearer identification
    EXECUTE 'COMMENT ON POLICY "Allow authenticated users to delete their own reports" ON storage.objects IS ''Allows authenticated users to delete reports from the reports bucket if they own the associated contract.''';
END;
$$;
