...
const ContractList: React.FC<ContractListProps> = ({ contractsToDisplay, onSelectContract, isSample = false }) => { // MODIFIED: Default isSample to false
  ...
  const handleDelete = async (contractId: string, filePath: string, event: React.MouseEvent) => {
    ...
    if (window.confirm('Are you sure you want to delete this contract and its analysis results? This action cannot be undone.')) {
      try {
      ...
      {contractsToRender.length === 0 ? (
        <Card>
          <CardBody className="text-center py-8">
            ...
            {contractsToDisplay === undefined && (
              <p className="text-sm text-gray-400 mt-1">Upload a contract to get started</p>
            )}
          </CardBody>
        </Card>
      ) : (
