...
const ContactForm: React.FC = () => {
  ...
  const handleSendOtp = async () => {
    ...
    if (!formData.email) {
      setOtpError(t('please_enter_email_address'));
      setOtpLoading(false);
      return;
    }
    ...
    try {
      const response = await fetch(`${import.meta.env.VITE_SUPABASE_URL}/functions/v1/send-email-otp`, {
        ...
        body: JSON.stringify({ email: formData.email, browserLanguage: i18n.language }),
      });

      if (!response.ok) {
        const errorData = await response.json();
        throw new Error(errorData.error || t('failed_to_send_otp'));
      }

      setOtpSent(true);
      setOtpMessage(t('otp_sent_to_email'));
    } catch (err: any) {
      setOtpError(err.message || t('error_sending_otp'));
  ...
  const handleVerifyOtp = async () => {
    ...
    if (!otpCode) {
      setOtpError(t('please_enter_the_otp'));
      setOtpLoading(false);
      return;
    }
    ...
    try {
      const response = await fetch(`${import.meta.env.VITE_SUPABASE_URL}/functions/v1/verify-email-otp`, {
        ...
        body: JSON.stringify({ email: formData.email, otp_code: otpCode, browserLanguage: i18n.language }),
      });

      if (!response.ok) {
        const errorData = await response.json();
        throw new Error(errorData.error || t('failed_to_verify_otp'));
      }

      setIsEmailVerified(true);
      setOtpMessage(t('email_verified_successfully'));
    } catch (err: any) {
      setOtpError(err.message || t('error_verifying_otp'));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!recaptchaToken) {
      setError(t('complete_recaptcha_verification'));
      setLoading(false);
      return;
    }
    ...
    try {
      ...
      if (!response.ok) {
        const errorData = await response.json();
        throw new Error(errorData.error || t('failed_to_submit_inquiry'));
      }
      ...
      setError(err.message || t('unexpected_error_occurred'));
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          <label htmlFor="firstName" className="sr-only">{t('first_name')}</label>
        ...
        <div>
          <label htmlFor="lastName" className="sr-only">{t('last_name')}</label>
          <div className="relative">
            <User className="absolute left-3 top-1/2 transform -translate-y-1/2 h-5 w-5 text-gray-400" />
            <input
              type="text"
      ...
      <div>
        ...
        <div className="relative flex items-center">
          ...
          >
            {otpLoading && !otpSent ? <Loader2 className="h-4 w-4 animate-spin" /> : t('verify_email')}
          </Button>
        </div>
        {otpError && (
      ...
      {otpSent && !isEmailVerified && (
        <div>
          <label htmlFor="otpCode" className="sr-only">{t('otp_code')}</label>
          <div className="relative flex items-center">
            ...
            <Button
              ...
              className="ml-2 rounded-r-md"
            >
              {otpLoading ? <Loader2 className="h-4 w-4 animate-spin" /> : t('verify_otp')}
            </Button>
          </div>
        </div>
      )}
