...
import { Mail, MessageSquare, Tag, AlertCircle, CheckCircle, User } from 'lucide-react'; // ADDED User icon
...
const SupportTicketForm: React.FC = () => {
  ...
  const [userName, setUserName] = useState<string>(''); // ADDED: State for user's full name
  const [loadingUserName, setLoadingUserName] = useState(true); // ADDED: Loading state for user name
    ...
    const fetchUserName = async () => {
      ...
      try {
        ...
        if (profileError) {
          ...
          setUserName('N/A'); // Fallback if profile fetch fails
        } else {
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      {success && (
        <div className="bg-green-100 border border-green-400 text-green-700 px-4 py-3 rounded relative flex items-center">
          ...
          <span>Your support ticket has been submitted successfully! We will get back to you soon.</span>
        </div>
      )}
      {error && (
      ...
      <div>
        ...
        <div className="relative">
          ...
          <input
            ...
            value={loadingUserName ? 'Loading...' : userName}
            readOnly // Make it read-only as it's from the user's profile
